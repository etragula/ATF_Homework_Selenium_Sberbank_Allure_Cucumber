...
public class MortgagePage extends BasePage {
    ...
    public MortgagePage fillFields(String whatFieldToFill, String value) {
        switchFrameCheck();
        ...
        Assert.assertEquals("Значение поля \"" + whatFieldToFill + "\" не соответсвует поданному значению!",
                value, field.getAttribute("value").replace(" ", ""));
        return this;
    }
...
//        scrollToElementJs(checkbox);
    ...
    public MortgagePage checkCalculationResults(String whatFieldToCheck, String value) {
        switchFrameCheck();
        ...
        if (whatFieldToCheck.equals("Процентная ставка")) {
            double actualValue = Double.parseDouble(field.getText()
                    .replace(",", ".")
                    .replace("%", ""));
            Assert.assertEquals(expectedValue, actualValue, 9e-10);
            return this;
        }
        Assert.assertEquals("Значение поля \"" + whatFieldToCheck + "\" не соответсвует \"" + value + "\"!",
                (int) expectedValue, fromStringToInteger(field.getText()));
        return this;
    }
